...
export class StockState {
  ...
  fetchStockData(
    { patchState }: StateContext<StockStateModel>,
    { payload }: StockDataAction
  ) {
    patchState({
      tableData: [],
      chartData: [],
    ...
    this.stockService.getStockDataRequest(payload).subscribe((response) => {
      const formatData = response.map((res) => {
        ...
        return {
          ...
          data: this.getStockData(res["Time Series (Daily)"], {
  ...
  private getStockData(
    ...
    return Object.keys(timeSeries)
      .filter((key) =>
        moment(key).isBetween(dates.startDate, dates.endDate, undefined, "[]")
      )
      .map((key) => {
        return {
          key: key,
